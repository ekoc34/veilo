...
export default function ProfilePage() {
  ...
  const [offMsgText, setOffMsgText] = useState('');
  const [offMsgSent, setOffMsgSent] = useState(false);
  const [offConvId, setOffConvId] = useState('');
  ...
  /* Tick now every 30 s for real-time online/offline labels */
  useEffect(() => {
    const t = setInterval(() => setNow(Date.now()), 30000);
    return () => clearInterval(t);
  }, []);

  /* Load offline conversations for profile owner's Berichten */
  useEffect(() => {
    if (!user || !isOwnProfile || !profileData?.uid) return;
    const q = query(collection(db, 'offConversations'), where('profileUid', '==', profileData.uid), orderBy('updatedAt', 'desc'));
    const unsubscribe = onSnapshot(q, (snap) => {
      setOfflineConvs(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });
    return () => unsubscribe();
  }, [user, isOwnProfile, profileData?.uid]);
  ...
  function formatLastSeen(lastSeen: any, nowMs: number): string {
    if (!lastSeen) return 'onbekend';
    const ts = lastSeen?.toDate ? lastSeen.toDate().getTime() : new Date(lastSeen).getTime();
    if (!ts || isNaN(ts)) return 'onbekend';
    const diff = Math.floor((nowMs - ts) / 1000);
    if (diff < 60) return `${diff} seconde${diff !== 1 ? 'n' : ''} geleden`;
    if (diff < 3600) { const m = Math.floor(diff / 60); return `${m} minuut${m !== 1 ? 'en' : ''} geleden`; }
    if (diff < 86400) { const h = Math.floor(diff / 3600); return `${h} uur${h !== 1 ? '' : ''} geleden`; }
    const d = Math.floor(diff / 86400); return `${d} dag${d !== 1 ? 'en' : ''} geleden`;
  }

  async function sendOfflineMessage() {
    if (!offMsgText.trim() || !profileData?.uid) return;
    const id = `${Date.now().toString(36)}_${Math.random().toString(36).substr(2, 8)}`;
    await setDoc(doc(db, 'offConversations', id), {
      profileUid: profileData.uid,
      profileUsername: profileData.username || username,
      senderLabel: `Anon-${id.substr(0, 7)}`,
      messages: [{ text: offMsgText.trim(), sentAt: new Date().toISOString(), from: 'anon' }],
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
    });
    setOffConvId(id);
  ...
  async function sendOfflineReply(convId: string) {
    if (!offReplyText.trim() || !profileData?.uid) return;
    const convRef = doc(db, 'offConversations', convId);
    const snap = await getDoc(convRef);
    if (!snap.exists()) return;
    const existing = snap.data().messages || [];
    await updateDoc(convRef, {
      messages: [...existing, { text: offReplyText.trim(), sentAt: new Date().toISOString(), from: 'profile' }],
      updatedAt: serverTimestamp(),
    });
    setOffReplyText('');
  }
      ...
      <div className="prof-header">
        <div className="prof-container">
          ...
          <ul>
            ...
            {!user && !isOwnProfile && (
              <li>
                <Link href="/" className="nav-btn nav-signup">
        ...
        <div className="modal-overlay" onClick={() => setShowMessages(false)}>
          <div className="modal-messages" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-messages-body">
              {activeOffConv ? (
                <div className="off-conv-view">
                  <div className="off-conv-view-head">
                    <strong>{activeOffConv.senderLabel} met jou</strong>
                    <a style={{ cursor: 'pointer', marginLeft: 'auto' }} onClick={() => setActiveOffConv(null)}>← Gelen Kutusu</a>
                  </div>
                  <div className="off-conv-messages">
                    {(activeOffConv.messages || []).map((m: any, i: number) => (
                      <div key={i} className={`off-conv-msg ${m.from === 'profile' ? 'mine' : 'theirs'}`}>
                        <strong>{m.from === 'profile' ? 'Jij' : activeOffConv.senderLabel}</strong>
                        <span className="off-conv-time">{formatLastSeen(m.sentAt, now)}</span>
                        <p>{m.text}</p>
                      </div>
                    ))}
                  </div>
                  <div className="off-conv-reply">
                    <input
                      type="text"
                      placeholder="Antwoord"
                      value={offReplyText}
                      onChange={(e) => setOffReplyText(e.target.value)}
                      onKeyDown={(e) => { if (e.key === 'Enter') sendOfflineReply(activeOffConv.id); }}
                    />
                    <button onClick={() => sendOfflineReply(activeOffConv.id)}>Verzenden</button>
                  </div>
                </div>
              ) : (
                <>
                  {offlineConvs.length === 0 && conversations.length === 0 ? (
                    <div className="modal-messages-empty">Geen berichten.</div>
                  ) : (
                    <ul>
                      {offlineConvs.map(conv => (
                        <li key={conv.id} className="modal-messages-item off-item" onClick={() => setActiveOffConv(conv)}>
                          <div className="modal-messages-info">
                            <strong>{conv.senderLabel} met jou</strong>
                            <span>Klik om gesprek te openen</span>
                            <span className="modal-messages-time">{formatLastSeen(conv.updatedAt, now)}</span>
                          </div>
                          <input type="checkbox" className="modal-messages-check" onClick={(e) => e.stopPropagation()} />
                        </li>
                      ))}
                      {conversations.map(conv => (
                        <li key={conv.id} className={`modal-messages-item${selectedConversations.has(conv.id) ? ' selected' : ''}`}
                          onClick={() => setSelectedConversations(prev => { const n = new Set(prev); if (n.has(conv.id)) n.delete(conv.id); else n.add(conv.id); return n; })}>
                          <img src="/images/default-avatar.svg" alt="" />
                          <div><strong>{conv.anonName || 'Anoniem'}</strong></div>
      ...
      <div className="prof-container prof-top">
        ...
        <div className="prof-main">
          ...
          {isOwnProfile ? (
            ...
            /* Offline profile panel */
            <div className="offline-panel">
              <h2 className="offline-title">MOMENTEEL OFFLINE</h2>
              <p className="offline-subtitle">Deze gebruiker is momenteel offline maar je kunt een bericht achterlaten.</p>
              <p className="offline-lastseen">Laatst gezien: {formatLastSeen(profileLastSeen, now)}</p>
              {!offMsgSent ? (
                <div className="offline-form">
                  <textarea
                    ...
                    value={offMsgText}
                    onChange={(e) => setOffMsgText(e.target.value)}
                    rows={5}
                  />
                  <div className="offline-form-row">
                    <button className="offline-send-btn" onClick={sendOfflineMessage} disabled={!offMsgText.trim()}>Verzenden</button>
                  </div>
                </div>
              ) : (
                <div className="offline-sent-wrap">
                  <div className="offline-arrow-col">
                    <span className="offline-arrow-icon">▶</span>
                    <span className="offline-sent-label">Bericht verstuurd</span>
                  </div>
                  <div className="offline-link-box">
                    <p className="offline-link-title">Gesprek link</p>
                    <a href={`/off/${offConvId}`} className="offline-link-url" target="_blank" rel="noopener noreferrer">
                      {typeof window !== 'undefined' ? `${window.location.origin}/off/${offConvId}` : `veilo.com/off/${offConvId}`}
                    </a>
                    <p className="offline-link-tip">Bewaar deze link in je browser, zo kan je de reactie lezen als er wordt geantwoord.</p>
                  </div>
                </div>
              )}
            </div>
